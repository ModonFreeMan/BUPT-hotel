...
public class RoomsServiceImpl implements RoomsService {
    ...
    private final static double[] attemperation = {0.3,0.6,1.2};
    ...
    public AirConditionerStatus getAirConditionerStatus(String roomId) {
        ...
        // 返回空调状态对象
        return new AirConditionerStatus(acServiceObject.getCurrentFee(),acServiceObject.getCurTem(),roomId,service_queue.contains(roomId),
                acServiceObject.getSpeedLevel(),acServiceObject.isSwitchStatus(),acServiceObject.getTargetTem(),
                acServiceObject.getTotalFee(),acServiceObject.isWorkMode());
    }
            ...
            if(!room_message.isSwitchStatus()) {// 现在是false,即突然关机的情况，需要删除请求队列和服务队列中的对应请求
                if(!service_queue.contains(request.getRoomId())) {// 非服务队列的情况
                    if(waiting_queue1.contains(request.getRoomId())||waiting_queue2.contains(request.getRoomId())||waiting_queue3.contains(request.getRoomId())){// 如果在等待队列需要加等待时间
                        ...
                        int waiting_length = (int)Duration.between(LocalDateTime.now(),
                                LocalDateTime.parse(room_message.getWaiting_queue_timestamp(),
                                        DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))).getSeconds();
    ...
    public boolean enterServiceQueue() {
        ...
        if (!roomId.equals("-1")){
            ...
            int request_length = (int)Duration.between(LocalDateTime.now(),
                    LocalDateTime.parse(room_message.getWaiting_queue_timestamp(),
                            DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))).getSeconds();
    ...
    public void leaveServiceQueue(String roomId, int leaveStatus) {
        ...
        double nowFee = (double)Duration.between(LocalDateTime.now(),
                LocalDateTime.parse(ACServiceMap.get(roomId).getService_queue_timestamp(),// 风速直接匹配费率下标
                        DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))).getSeconds()/60*centralACStatus.getFeeRate()[(int)ACServiceMap.get(roomId).getSpeedLevel()];
        // 通知信息计算处理详单
        // todo:告诉消息队列，生成详单
//        new DetailedBill(ACServiceMap.get(roomId).getCurTem(), LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")),nowFee,centralACStatus.getFeeRate(),roomId,ACServiceMap.get(roomId).getSpeedLevel(),ACServiceMap.get(roomId).getBeforeServiceTem(),ACServiceMap.get(roomId).getWaiting_queue_timestamp());
    ...
    public void enterWaitQueue(String roomId) {
        ...
        waiting_queue2.remove(roomId);
        waiting_queue3.remove(roomId);
        // 假设风速对三个值分别对应0，1，2，也就是没有打开空调控制风速为0的选项
        switch ((int)ACServiceMap.get(roomId).getSpeedLevel()) {
            case 0:
    ...
    public void scheduledTask() {
        ...
        for (String roomId:service_queue
            ...
            double Temperature_variation = attemperation[(int)ACServiceMap.get(roomId).getSpeedLevel()]/6;
            // 先判断是否关机的，直接移出
